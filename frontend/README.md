...
- **Angular frontend** (`frontend/`): a public landing page, sign-in/sign-up, the chat interface (with a chat history sidebar, at `/chat` and `/chat/:chatId`), and an admin page for managing user roles.
...
The frontend's chat page (`/chat`, `/chat/:chatId`) is a Claude/Gemini-style two-pane layout: a history sidebar backed by `GET /chats/`, and a conversation pane backed by `GET /chats/{id}` and `POST /chats/{id}/messages`. Starting a message with no chat selected creates one first, chats can be renamed or deleted from the sidebar, and the sidebar itself collapses to a thin rail.
